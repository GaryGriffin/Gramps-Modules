...
#CONFIG.register('map.draw-notes-segments',1)
CONFIG.register('map.include-citation-notes',0)


#CONFIG.register('map.draw-DNA-segments',0)
...
class DNASegmentMap(Gramplet):
    ...
    def build_options(self):
        ...
        segment_source = _("Source of DNA Segments")
        self.opt_source = EnumeratedListOption(segment_source, self.source_mode)
        self.opt_source.add_item("Notes", _("Association Notes"))
        self.opt_source.add_item("DNA", _("DNASegments"))
        self.add_option(self.opt_source)
    ...
    def save_options(self):
        ...
        self.source_mode = self.opt_source.get_value()
...
#
    def _get_chromosome_side(self,active,associate):
        ...
        return side
#
# Read all DNASegments and build segment array
#
    def _write_DNASegments_chromo(self, segment, side, rgb_color, associate, note, segmap, Match_Account_Name):
        PHASE_MATERNAL = 2
        PHASE_PATERNAL = 3
        if associate:
            ...
            id_str = Match_Account_Name
        chromo = segment.get_chromosome()
        start = segment.get_start_bp()
        stop = segment.get_end_bp()
        cms = segment.get_shared_cm()
        snp = segment.get_snp_count()
#
...
# Override the maternal/paternal flag if phase specified in Segment and Tree does not find a relationship
        updated_side = side
        if side == 'U':
            if phase == PHASE_MATERNAL:
                updated_side = 'M'
            elif phase == PHASE_PATERNAL:
                updated_side = 'P'
        if associate:
            handle = associate.get_handle()
        else:
            handle = None
# Add the segment to the drawing queue
        segmap.segments.append([chromo, start, stop, updated_side, cms, snp, id_str, rgb_color, associate, handle, note])

    def _write_notes_chromo(self, line, side, rgb_color, assoc, note, segmap):
#
# Read all Association Notes and build segment array
#
        if "\t" in line:
# Tabs are the field separators. Now determine THOUSEP and RADIXCHAR. 
#       Use Field 2 (Stop Pos) to see if there are THOUSEP there. 
#       Use Field 3 (SNPs) to see if there is a radixchar
            field = line.split('\t')
            if len(field) > 3:
                if "," in field[2]:
                    line = line.replace(",", "")
...
# If Tab is not the field separator, then comma is. And point is the radixchar.
        field = line.split(',')
#    Set up args and append to segments
        if len(field) < 4:
            return False
        ...
        try:
            ...
            snp = 0
        seg_comment = ''
# Override side if specified in notes
        updated_side = side
        if len(field) > 5:
            if field[5] in { "M", "U"}:
...
# fixed colormap (partial) to use to MyHeritage color based on grandparent instead of association name
    ...
    def _build_grandparent_table(self,mother,father):
        grand_handle = {}
        if mother :
    ...
    def _add_title_to_graph(self,cr):
    ...
    def _add_chromosome_labels_to_graph(self,cr, height):
        chr_height = 12
        spacing = 2
        label_width = 0
        offset = height + 5
    ...
    def _add_ticks_to_graph(self,cr,label_width, offset):
        chr_height = 12
        spacing = 2
        allocation = self.get_allocation()
    ...
    def _add_background_to_graph(self,cr,i, chromo, label_width, offset, centloc):
        chr_height = 12
        spacing = 2
        allocation = self.get_allocation()
        chart_width = (allocation.width - label_width) * 0.95
        ...
        cr.rectangle(label_width+chart_width * centloc / self.maximum,
                 i * 2 * (chr_height + spacing) + offset,
                 chart_width * (chromo-centloc) / self.maximum,
                 chr_height)
    ...
    def _add_single_background_to_graph(self,cr,row_num,this_chromo, label_width, offset, centloc):
        chr_height = 12
        spacing = 2
        allocation = self.get_allocation()
        chart_width = (allocation.width - label_width) * 0.95
...
# Define grandparents and great grandparents for visual phasing. Trap if user changed tree while in this view
        try:
            ...
            return
        self._build_grandparent_table(mother, father)
# Title
        height = self._add_title_to_graph(cr)
        offset = height + 5
# Adjust max width for single chromosome view to max of that chromosome
        if draw_single_chromosome :
...
# Chromosome labels
        ...
        if not draw_single_chromosome :
            label_width = self._add_chromosome_labels_to_graph(cr, height)
...
# Ticks and grid lines
        if not draw_single_chromosome :
            self._add_ticks_to_graph(cr, label_width, offset)
            offset += spacing
...
# Chromosomes background
    ...
    # draw paternal
                self._add_background_to_graph(cr,i, chromo[1], label_width, offset, centloc)
    ...
    # draw Maternal
                self._add_background_to_graph(cr,i, chromo[1], label_width, offset+chr_height, centloc)
    ...
    # Grey out paternal X background for males
        ...
        if draw_single_chromosome:
          ...
          for chromo, start, stop, side, cms, snp, assoc_name, rgb_color, associate, handle, note in self.segments:
            chromo_count += 1
            try:
                this_chromo = self.labels.index(chromo.strip())
            except ValueError:
                continue
            if chromo.strip() == current_chromosome:
              if assoc_name not in legend_name_draw:
                last_name = assoc_name
                legend_name_draw.append(assoc_name)
                row_num += 1
    # Background
                if self.show_centromere :
    ...
    # draw paternal
                self._add_single_background_to_graph(cr,row_num, this_chromo, label_width, offset, centloc)
                cr.set_source_rgba(self.paternal_background[0], self.paternal_background[1], self.paternal_background[2], self.paternal_background[3])
                cr.fill_preserve()
                cr.stroke()
    # draw maternal
                self._add_single_background_to_graph(cr,row_num, this_chromo, label_width, offset + chr_height, centloc)
                cr.set_source_rgba(self.maternal_background[0], self.maternal_background[1], self.maternal_background[2], self.maternal_background[3])
                cr.fill_preserve()
                cr.stroke()
    # Grey out paternal X background for males
...
# Segment Info
              RowNum = row_num
              RowNum = legend_name_draw.index(assoc_name) + 1
# RowNum is the index of assoc_name in legend_name_draw
              chr_offset = RowNum * 2 * (chr_height + spacing) + offset
              chr_mult = 1
              if side[0] == 'M':
...
#
          ...
          for chromo, start, stop, side, cms, snp, assoc_name, rgb_color, associate, handle, note in self.segments:
            chromo_count += 1
            try:
                i = self.labels.index(chromo.strip())
            except ValueError:
                continue
...
# Legend entry
#
            if (assoc_name not in legend_name_draw) and (not draw_grandparent_color) :
                legend_name_draw.append(assoc_name)
                legend_count += 1
                last_name = assoc_name
                cr.rectangle(legend_offset_x - legend_chr_height - 2 * spacing,
...
# If Match is not in db, drop the relationship tooltip
                if self.segments[active][8]: 
        ...
        if not draw_grandparent_color :
            for i, rect in enumerate(self.__legendrects):
                if (event.x > rect[0] and event.x < rect[0] + rect[2] and
                        event.y > rect[1] and event.y < rect[1] + rect[3]):
                    active = i
            if (active != -1) and self.__assoc_handle[active]:
